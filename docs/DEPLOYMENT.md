...
在 Agent workspace 的 `HEARTBEAT.md` 中添加以下任务块。
...
#### 1. 查自己加入的项目
`mai --project <project-path> project list --agent <agent-name>`
无返回 → 结束

#### 2. 查自己负责的 issue
`mai --project <project-path> queue check --handler <agent-name>`
无返回 → 结束

#### 3. 查看 issue
`mai issue show <issue-id>`

取最高优先级（P0>P1>P2）issue，跳过 blocked / overdue>30min 的 issue

#### 4. 认领并更新进展
`mai --project <project-path> issue claim <issue-id> -o <agent-name>`
...
### 2. 读取摘要：
`daily-summary read <agent-name>`

先读取，再更新

### 3. 写入摘要: 

`mai daily-summary write <agent-name> <content>`
