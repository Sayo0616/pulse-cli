...
1. 检查 <HANDLER_X> 队列是否有待处理 issue (包含转交给自己的):
   mai --project <SHARED_WORKSPACE_PATH> queue check <HANDLER_X> --handler @<AGENT_NAME>

2. 若有待认领 issue,认领并处理:
   mai --project <SHARED_WORKSPACE_PATH> issue claim <issue-id>
   # [执行处理工作] ...
   mai --project <SHARED_WORKSPACE_PATH> issue complete <issue-id> <处理结论>
   mai --project <SHARED_WORKSPACE_PATH> issue submit-to-creator <issue-id>
```

#### 3.1.2.1 多步协作流转 (v1.7)

当任务需要多个 Agent 依次处理或需要发起方确认时，按以下逻辑配置心跳：

- **处理人 A**: `claim` -> `complete` -> `transfer <HANDLER_B>`
- **处理人 B**: `claim` -> `complete` -> `submit-to-creator`
- **发起方 (Creator)**: 监控自己的队列或特定 Issue 状态，执行 `confirm` 或 `reject`。

---

### 3.1.3 配置锁监护 cron
完整流程:trigger → 各 Agent 独立执行 write → status 查看进度 → reset 重置（如需）
