...
> 此命令会自动在 `config.json` 中注册 Agent,并创建与 Agent 名相同的任务队列。
...
| 字段 | 说明 |
|---|---|
| `root` | 超级管理员列表（字符串或数组）。未配置时默认取操作系统用户名。 |
| `queues.<name>.owner` | 负责该队列的 Agent 名称(必须与 `agents` 中的 key 一致)。v1.9.0 后 `handler` 字段建议改为 `owner`。 |
...
> Skill 文件随 mai-cli 包分发。将 Skill 复制到所有添加的Agent的工作区。
...
在 Agent workspace 的 `HEARTBEAT.md` 中添加以下任务块。注意：所有写操作均需携带 `-o` / `--operator <name>` 参数（REQ-A）。

```markdown
## Mai CLI 任务流

### 1. 处理人流程 (Handler Flow)
- **发现任务**: `mai queue check --handler <AGENT_NAME>`
- **处理任务**:
  1. 认领: `mai issue claim <issue-id> --operator <AGENT_NAME>`
  2. 报告进度 (修订): `mai issue amend <issue-id> "工作进展..." --operator <AGENT_NAME>`
  3. 遇到阻塞: `mai issue block <issue-id> "阻塞原因..." --operator <AGENT_NAME>`
  4. 解除阻塞: `mai issue unblock <issue-id> --operator <AGENT_NAME>`
  5. 转交验收: `mai issue transfer <issue-id> <OWNER_NAME> --operator <AGENT_NAME>` (转交给队列负责人验收)

### 2. 负责人/管理员流程 (Owner/Root Flow)
- **创建任务**: `mai issue new <queue> "标题" --operator <AGENT_NAME>`
- **结项验收**:
  1. 通过结项: `mai issue complete <issue-id> "验收通过" --operator <AGENT_NAME>` (或使用 alias `confirm`)
  2. 拒绝退回: `mai issue reject <issue-id> "退回原因" --operator <AGENT_NAME>`
  3. 重新打开: `mai issue reopen <issue-id> "原因" --operator <AGENT_NAME>`

### 3. 每日摘要
- 检查进度: `mai daily-summary status`
- 写入摘要: `mai daily-summary write <AGENT_NAME> <汇报内容...>`
```
...
mai --project <SHARED_WORKSPACE_PATH> issue new \
  <HANDLER_A> "部署验证测试" --ref TEST-001 --operator "deployer"
```
...
```bash
# 列出队列,确认 issue 出现
mai --project <SHARED_WORKSPACE_PATH> issue list <HANDLER_A>

# 认领
mai --project <SHARED_WORKSPACE_PATH> issue claim <ISSUE-ID> --operator <AGENT_NAME>

# 完成 (需以 owner 身份或 root 身份执行)
mai --project <SHARED_WORKSPACE_PATH> issue complete <ISSUE-ID> "验证通过" --operator <OWNER_NAME>
```
