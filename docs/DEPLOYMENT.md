...
| **Handler** | Issue 的当前实际处理人（owner）。Issue 创建时的默认 handler 来自队列配置，但之后可通过 `transfer`/`submit-to-creator`/`reject` 等命令变更，queue 始终不变。 |
...
在 Agent workspace 的 `HEARTBEAT.md` 中添加以下任务块。建议保持简洁，方便 Agent 循环执行：
...
- **处理任务**:
  1. 认领: `mai issue claim <issue-id>`
  2. 完成并记录: `mai issue complete <issue-id> "处理结论"`
  3. 提交确认: `mai issue submit-to-creator <issue-id>`
  4. (或转交): `mai issue transfer <issue-id> <next-handler>`

### 2. 发起人验收 (Creator Flow)
- **扫描回传**: `mai queue check --handler <AGENT_NAME>`
- **完成验收**:
  1. 通过结项: `mai issue confirm <issue-id>`
  2. 拒绝退回: `mai issue reject <issue-id> "退回原因"`
...
# Issue
mai --project <PATH> issue new <queue> <title> [--ref <ref-id>] [--priority P0|P1|P2]
