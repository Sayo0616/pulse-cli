...
### 步骤 1.2：配置 config.json

在 `<SHARED_WORKSPACE_PATH>/.mai/config.json` 中写入团队配置：

```json
{
  "queues": {
    "<HANDLER_A>-questions": { "handler": "<HANDLER_A>", "sla_minutes": 120, "id_prefix": "<PREFIX_A>" },
    "<HANDLER_B>-questions": { "handler": "<HANDLER_B>", "sla_minutes": 120, "id_prefix": "<PREFIX_B>" }
  },
  "agents": {
    "<HANDLER_A>": { "heartbeat_minutes": 17 },
    "<HANDLER_B>": { "heartbeat_minutes": 29 }
  },
  "daily_summary_order": ["<HANDLER_A>", "<HANDLER_B>"]
}
```
...
|---|---|
| `<HANDLER_A>` | `mai --project <PATH> issue *` + `queue check <HANDLER_A>-questions` + `daily-summary write/read` |
| `<HANDLER_B>` | `mai --project <PATH> issue *` + `queue check <HANDLER_B>-questions` + `daily-summary write/read` |
