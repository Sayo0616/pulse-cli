...
- **UX: 优化处理记录展示** — `mai issue show` 和 `mai issue status` 现在能完美展示包含多行文字、代码块或复杂 Markdown 的处理备注，并保持整齐的缩进。
- **Fix: 修正冲突升级报告** — 修正了 `mai escalation gen` 在处理新的结构化 timeline 时输出原始字典的问题。

## v1.10.7 (2026-05-02)
- **Fix: 修复 `issue show` 换行符丢失与缩进问题** — 修正了处理记录（timeline）解析时因过度去除空白导致的换行符丢失，并优化了渲染逻辑，确保多行备注能保持正确的缩进和格式。
- **Docs: 优化心跳指令参考** — 更新了 `DEPLOYMENT.md` 中的心跳流程（HEARTBEAT），增加了更清晰的项目列表和工单处理步骤指导。
- **Docs: 增强 `SKILL.md` 指导** — 明确了 Issue 处理完成后必须先追加完成信息再进行转交的要求。
