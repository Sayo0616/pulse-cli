...
def test_default_queues_keys():
    from pulse.config import DEFAULT_QUEUES
...
def test_global_args_defaults():
    from pulse.config import GlobalArgs
    g = GlobalArgs()
...
def test_get_heartbeat_intervals():
    from pulse.config import get_heartbeat_intervals, DEFAULT_AGENTS
...
def test_get_queue_sla():
    from pulse.config import get_queue_sla, DEFAULT_QUEUES
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
...
def test_exec_safe_check_safe():
    from pulse.safe_exec import exec_safe_check
...
def test_make_issue_content():
    from pulse.issue import make_issue_content
...
def test_parse_issue_file():
    from pulse.issue import parse_issue_file
    import tempfile
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_next_issue_id():
    from pulse.issue import next_issue_id
    import tempfile
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        pulse_dir = root / ".pulse" / "queues" / "programmer-questions"
        pulse_dir.mkdir(parents=True)
        (pulse_dir / "REQ-001.md").write_text("# [REQ-001] First\n")
        (pulse_dir / "REQ-002.md").write_text("# [REQ-002] Second\n")
        assert next_issue_id(root, "programmer-questions") == "REQ-003"


def test_read_issue_not_found():
    from pulse.issue import read_issue
    import tempfile
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
...
def test_lock_path():
    from pulse.lock import lock_path
    import tempfile
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        lp = lock_path(root, "REQ-001")
        assert lp.name == "REQ-001.lock"
        assert ".pulse/locks" in str(lp)


def test_acquire_and_release_lock():
    from pulse.lock import acquire_lock, release_lock, check_lock
    import tempfile
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        (root / ".pulse" / "locks").mkdir(parents=True)
...
def test_ensure_pulse_structure():
    from pulse.project import ensure_pulse_structure
    import tempfile
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        ensure_pulse_structure(root)
        pulse = root / ".pulse"
        assert (pulse / "queues").exists()
        assert (pulse / "locks").exists()
        assert (pulse / "processing").exists()
        assert (pulse / "decisions").exists()
        assert (pulse / "history").exists()
        assert (pulse / "events").exists()
...
def test_out_text(capsys):
    from pulse.pulse import out, GLOBAL
    old = GLOBAL.format
    GLOBAL.format = "text"
    out("hello world")
...
def test_err_exits(capsys):
    from pulse.pulse import err, GLOBAL
    old = GLOBAL.format
    GLOBAL.format = "text"
...
def test_dispatch_project_init_unknown_project(capsys):
    from pulse.pulse import build_parser, GLOBAL
    old = GLOBAL.format
    GLOBAL.format = "text"
    parser = build_parser()
    ...
    GLOBAL.dry_run = True
    from pulse.pulse import dispatch
    try:
        dispatch(args)
    except SystemExit:
        pass
